...
func getLatestHeight(native *native.NativeContract) uint64 {
	height, _ := zilliqa.GetCurrentTxHeaderHeight(native, ZILChainID)
	return height
}

func getHeaderHashByHeight(native *native.NativeContract, height uint64) []byte {
	block, _ := zilliqa.GetTxHeaderByHeight(native, height, ZILChainID)
	return block.BlockHash[:]
}

func TestSyncGenesisHeaderZilliqa(t *testing.T) {
	...
	txBlockAndDsComm := &zilliqa.TxBlockAndDsComm{
		TxBlock: &txblock,
		DsBlock: &dsblock,
		DsComm:  initComm,
	}
	...
	n, err := NewNative(scom.MethodSyncGenesisHeader, param)
	if err != nil {
	...
	assert.Equal(t, "38288732b2e67ec268e6b1a6f1c3b57748e6b166ab793aa3298b125c8ae76c27", ziutil.EncodeHex(headerHash))
}
...
func TestSyncGenesisHeader_ParamError(t *testing.T) {
	...
	n, _ := NewNative(scom.MethodSyncGenesisHeader, param)
	handler := zilliqa.NewHandler()
	err := handler.SyncGenesisHeader(n)
	assert.Equal(t, 1, typeOfError(err), err)
}

func TestSyncBlockHeaderZilliqa(t *testing.T) {
